...
func (l *Logic) ToWebP(ctx context.Context, request *pb.ToWebPRequest, response *pb.ToWebPResponse) error {
	...
	filename := fmt.Sprintf("%s%s%s/%s", rootPath, filePath, path, uri[len(uri)-1])
	...
	var img image.Image
	fileType, err := FileTypeGet(filename)
	if err != nil {
		fmt.Println("err,", err)
	}
	switch fileType {
	...
	newFile := fmt.Sprintf("%s%s%s/new-%s.webp", rootPath, filePath, path, uri[len(uri)-1])
	...
	response.Path = fmt.Sprintf("%s%s/new-%s.webp", filePath, path, uri[len(uri)-1])
